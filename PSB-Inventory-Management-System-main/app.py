...
class Login:
    ...
    def __init__(self, master):
        ...
        self.title = Label(self.frame, text='''PSB Inventory Management System''', pady=50, font='Courier 25 bold')
        self.title.grid(row=0, column=0)

        
        self.subtitle = Label(self.frame, text='''Login''', pady=40, font='Helvetica 18 bold')
        self.subtitle.grid(row=1, column=2)
        ...
        self.usernameText.grid(row=3, column=3)

        self.passwordText = Entry(self.frame)
        self.passwordText.grid(row=4, column=3)

        self.loginButton = Button(
            self.frame, text="Login", command=self.authLogin)
        self.loginButton.grid(row=5, column=2, padx=10, pady=10)
